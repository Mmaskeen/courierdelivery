            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card">
                        ...
                        <form method="post" id="editOrderForm" action="{{route('admin_update_order',$order->id)}}" enctype="multipart/form-data" onsubmit="myButton.disabled = true; return true;">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        {{--                                        {{dd(old('customer_address'))}}--}}
                                        <div class="form-group">
                                            <label class="form-label">Address</label>
                                            <input type="text" class="form-control" value="{{old("customer_address") ?? $order->address}}" name="customer_address" placeholder="Address">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control cities" name="status" data-placeholder="Choose Status"
                                             {{-- {{ $order->status == 'Dispatched' ? "disabled" : "" }} --}}
                                             >
                                    ...
                                    <input type="hidden" value=""  name="printInvoice">

                                </div>
                            </div>
                            <div class="card-footer text-right">
                                @if($order->status == 'Dispatched' && $order->dispatch_by == "Tcs" || $order->status == 'Dispatched' && $order->dispatch_by == "Stallion")
                                    <button class="btn btn-primary" href="##" onClick="history.go(-1); return false;">Go back</button>
                                @elseif($order->status == 'Dispatched' && $order->dispatch_by !== "Tcs" || $order->status == 'Dispatched' && $order->dispatch_by !== "Stallion")
                                    <button class="btn btn-primary" href="##" onClick="history.go(-1); return false;">Go back</button>
                                    <button type="submit" name="myButton" id="submitFormButton" class="btn btn-primary" >Update Order</button>
                                @else
                                    <button type="submit" name="myButton" id="submitFormButton" class="btn btn-primary" >Update Order</button>
                                @endif
                            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click','#submitFormButton',function (e) {
                e.preventDefault();
               let status = $('select[name="status"]').children("option:selected").val();
               if(status === 'Dispatched'){
                   swal.fire({
                       ...
                       if (result.value) {
                           $('input[name="printInvoice"]').val('printInvoice');
            ...
            $('select[name="dispatch_by"]').on('change',function (e) {
                ...
                if(val !== ''){
                    ...
                    let url = '{{route('admin_get_dispatch_city')}}';
                    let data = {
                        'type' : val,
                    };
